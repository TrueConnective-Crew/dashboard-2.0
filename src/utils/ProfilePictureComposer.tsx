...
function AvatarComposer({ avatarUrl }: Props) {
  ...
  function handleDownloadAll(): void {
    try {
      ...
      triggerDownload(canvasRef_orange.current, `tc-crew-kuerbis.png`);
      triggerDownload(canvasRef_red.current, `tc-crew-horror.png`);
    } catch (e) {
      ...
      <Stack direction={{ xs: "column", md: "row" }} spacing={4} alignItems={{ xs: "center", md: "flex-start" }}>
        ...
        <Stack direction={"column"} spacing={4}>
          <canvas
            ...
            style={{ width: "256px", height: "256px" }}
          />
          <Typography textAlign={"center"}>Horror</Typography>
        </Stack>
      </Stack>

      <Dropdown>
        <MenuButton>Download Profilbilder</MenuButton>
        <Menu>
          <MenuItem onClick={handleDownloadAll}>Alle Profilbilder</MenuItem>
          <ListDivider />
          <MenuItem onClick={() => handleDownloadVariant("original")}>
            Original
          </MenuItem>
          <MenuItem onClick={() => handleDownloadVariant("orange")}>
            Kürbis
          </MenuItem>
          <MenuItem onClick={() => handleDownloadVariant("red")}>
            Horror
          </MenuItem>
        </Menu>
      </Dropdown>
